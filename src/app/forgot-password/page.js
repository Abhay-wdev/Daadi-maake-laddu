...
export default function ForgotPassword() {
  ...
  const handleSendOTP = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`https://dadimaabackend-2.onrender.com/api/auth/forgot-password`, {
        method: "POST",
  ...
  const handleResetPassword = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`https://dadimaabackend-2.onrender.com/api/auth/reset-password`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },

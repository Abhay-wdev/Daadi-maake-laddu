...
const ProductDetails = ({ product }) => {
  ...
  const { cart, addItem, updateItem, removeItem, fetchCart, loading: cartLoading } =
    useCartStore();

  const isInCart = (productId) =>
    cart?.items?.some((item) => item.product._id === productId);

  // 🛒 Fetch cart when user & token are available
  useEffect(() => {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-orange-50 to-amber-50">
        <div className="text-center p-8 bg-white rounded-2xl shadow-xl max-w-md border border-orange-100">
          ...
          <Link
            href="/products"
            className="inline-block bg-gradient-to-r from-orange-500 to-red-500 text-white px-6 py-3 rounded-xl font-medium hover:from-orange-600 hover:to-red-600 transition-all shadow-lg"
          >
            Browse Products
          </Link>
        </div>
  ...
  const handleAddToCart = async () => {
    ...
    // 👇 If NOT logged in → send to login instead of adding
    if (!isLoggedIn || !token) {
                      ...
                      onClick={() => {
                        const shareData = {
                          ...
                          text:
                            ...
                            "Found something interesting on our store!",
                ...
                <div className="mt-3 flex items-center text-sm">
                  ...
                  {product.availabeQuantity > 0 && (
                    <span className="ml-2 text-gray-600">
                      Only {product.availabeQuantity} left
                    </span>
                  )}
                </div>
              </div>
              ...
              {product.weight && (
                <div className="bg-orange-50 rounded-xl p-4 border border-orange-100">
                  <h3 className="font-medium text-gray-900 mb-2 flex items-center gap-2">
                    ...
                    Weight{" "}
                    <p className="text-gray-700 ml-1">{product.weight}</p>
                  </h3>
                </div>
              )}
              ...
              {product.nutritionalInfo && (
                <div className="bg-white rounded-2xl p-6 shadow-sm border border-orange-100">
                  ...
                  <div className="space-y-3">
                    ...
                    {product.nutritionalInfo.values &&

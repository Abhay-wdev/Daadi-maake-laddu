...
export default function UserList() {
  ...
  const fetchUsers = async () => {
    try {
      setLoading(true);
      const res = await fetch("https://dadimaabackend-1.onrender.com/api/user", {
        headers: {
  ...
  const handleDelete = async (userId, userRole) => {
    ...
    try {
      const res = await fetch(`https://dadimaabackend-1.onrender.com/api/user/${userId}`, {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${token}`,
        },
  ...
  const handleToggleStatus = async (userId, currentStatus, userRole) => {
    ...
    try {
      const res = await fetch(
        `https://dadimaabackend-1.onrender.com/api/user/${userId}/status`,
        {
          method: "PATCH",
          headers: {
